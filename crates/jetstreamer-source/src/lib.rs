...
impl VixenStreamHandler {
    ...
    fn precalculate_filters(filters: &Filters) -> (Vec<String>, Vec<String>) {
        ...
        for (filter_id, prefilter) in &filters.parsers_filters {
            ...
            // Skip instruction parsers for transaction updates
            ...
            // If any account filters exist, we should process this transaction avoiding twice process -<
    ...
    async fn process_block(&self, block: BlockData) -> Result<(), Error> {
        ...
        match block {
            BlockData::Block {
                ...
                let count = self.blocks_processed.fetch_add(1, Ordering::Relaxed);
                if count % 10_000 == 0 && count > 0 {
                    info!(slot, count, "Processed blocks (found matches)");
                }
