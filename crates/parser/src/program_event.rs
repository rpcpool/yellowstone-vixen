//! Composable wrapper for parsing program events alongside regular instructions.
//!
//! Programs emit events two ways:
...
//!
//! [`ProgramEventParser`] wraps an instruction parser and an event parser
//! (both generated by `include_vixen_parser!`) to handle both patterns automatically.
...
pub struct ProgramEventParser<InstructionParser, EventParser> {
    instruction_parser: InstructionParser,
    event_parser: EventParser,
    program_id: Pubkey,
}

impl<InstructionParser, EventParser> ProgramEventParser<InstructionParser, EventParser> {
...
fn is_cpi_event(data: &[u8]) -> bool { data.len() >= 8 && data[..8] == EVENT_IX_TAG }
...
impl<InstructionParser, EventParser> Parser
    for ProgramEventParser<InstructionParser, EventParser>
where
    ...
    EventParser::Output: Send,
{
    ...
    fn parse(
        ...
        async move {
            ...
            if is_cpi_event(&value.data) && value.program == self.program_id {
                return Err(ParseError::Filtered);
            }

            // 1. Regular instruction: delegate to the instruction parser.
            //    Non-Filtered errors are treated as "no instruction" rather than
            //    fatal — the IDL may not cover every variant (e.g. Jupiter adds
            //    new swap variants frequently), but we still want the events.
            ...
            for inner in &value.inner {
                if is_cpi_event(&inner.data) && inner.program == self.program_id {
