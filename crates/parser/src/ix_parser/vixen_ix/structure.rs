...
use spl_pod::solana_program::{
    instruction::CompiledInstruction, message::AccountKeys, program_error::ProgramError,
    pubkey::Pubkey,
};
...
use yellowstone_vixen_core::{ParseResult, TransactionUpdate, Update, UpdateType};

use super::helpers::{get_account_from_index, *};

// pub struct ReadableIxs {
//     pub index: u32,
//     pub ixs: Vec<ReadableIxs>,
// }

// impl ReadableIxs {
//     fn try_from_inner_ix(
//         inner_ix: InnerInstruction,
//         current_stack_depth: u32,
//     ) -> Result<ReadableIxs, String> {
//         Ok(ReadableIxs {
...
// impl TryFrom<InnerInstructions> for ReadableIxs {
//     type Error = String;

//     fn try_from(inner_ixs: InnerInstructions) -> Result<Self, String> {
//         let mut current_stack_depth = 1u32;
//         let instructions = inner_ixs
//             .instructions
//             .into_iter()
//             .enumerate()
//             .map(|(idx, ix)| ReadableIxs::try_from_inner_ix(ix, current_stack_depth))
//             .collect::<Result<Vec<ReadableIxs>, String>>()?;
//         Ok(ReadableIxs {
...
impl InstructionUpdate {
    pub fn try_from(tx_update: TransactionUpdate) -> Result<Self, String> {
        ...
        let writeable_addresses = tx_meta.loaded_writable_addresses.to_pubkey_vec()?;
        let tx_account_keys =
