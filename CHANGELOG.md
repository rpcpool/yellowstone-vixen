...
- **Breaking** for custom sources: `SourceTrait` no longer has a `Config` associated type or a `new` constructor. `connect` (and `connect_with_filter_updates`) now receive the `Filters` derived from the registered pipelines, and the runtime is handed a source value through the new `RuntimeBuilder::try_build_with(source, buffer)`. Sources that should be constructed from a `ShipsternConfig` section implement the new `FromConfig` trait, which is what keeps `try_build(config)` and `build(config)` working unchanged for the built-in sources. Every source in the repository implements it, so existing `main` functions and config files are unaffected; only out-of-tree `SourceTrait` implementations need updating, and test doubles no longer need a stub constructor. The `StreamBuilder` in `shipstern-stream` gains the same `try_build_with` / `build_with` pair.
